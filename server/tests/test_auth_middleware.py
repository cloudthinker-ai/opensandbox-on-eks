...
def _build_test_app_with_proxy_auth():
    app = FastAPI()
    config = AppConfig(
        server=ServerConfig(api_key="secret-key", proxy_auth_required=True),
        runtime=RuntimeConfig(type="docker", execd_image="opensandbox/execd:latest"),
        ingress=IngressConfig(mode="direct"),
    )
    app.add_middleware(AuthMiddleware, config=config)
...
def test_auth_middleware_proxy_auth_required_blocks_unauthenticated():
    """When proxy_auth_required is True, proxy routes require authentication."""
    app = _build_test_app_with_proxy_auth()
    client = TestClient(app)
    response = client.get("/sandboxes/abc/proxy/8080/foo")
    assert response.status_code == 401


def test_auth_middleware_proxy_auth_required_allows_authenticated():
    """When proxy_auth_required is True, authenticated proxy requests succeed."""
    app = _build_test_app_with_proxy_auth()
    client = TestClient(app)
    response = client.get(
        "/sandboxes/abc/proxy/8080/foo",
        headers={"OPEN-SANDBOX-API-KEY": "secret-key"},
    )
    assert response.status_code == 200
    assert response.json()["proxied"] is True
