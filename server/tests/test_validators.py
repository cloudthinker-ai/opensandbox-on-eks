...
# ============================================================================
# Egress Configured Validation Tests
# ============================================================================


class TestEnsureEgressConfigured:
    """Tests for ensure_egress_configured validator."""

    def test_no_network_policy_passes(self):
        """None network_policy should not raise."""
        ensure_egress_configured(None, None)

    def test_with_policy_and_image_passes(self):
        """Valid policy + configured image should not raise."""
        from src.api.schema import NetworkPolicy
        from src.config import EgressConfig

        policy = NetworkPolicy(default_action="deny")
        egress_cfg = EgressConfig(image="ghcr.io/egress-sidecar:latest")
        ensure_egress_configured(policy, egress_cfg)

    def test_with_policy_no_egress_config_raises(self):
        """Policy present but egress_config=None should raise 400."""
        from src.api.schema import NetworkPolicy

        policy = NetworkPolicy(default_action="deny")
        with pytest.raises(HTTPException) as exc_info:
            ensure_egress_configured(policy, None)
        assert exc_info.value.status_code == 400
        assert exc_info.value.detail["code"] == SandboxErrorCodes.INVALID_PARAMETER

    def test_with_policy_no_image_raises(self):
        """Policy present but egress_config.image=None should raise 400."""
        from src.api.schema import NetworkPolicy
        from src.config import EgressConfig

        policy = NetworkPolicy(default_action="deny")
        egress_cfg = EgressConfig(image=None)
        with pytest.raises(HTTPException) as exc_info:
            ensure_egress_configured(policy, egress_cfg)
        assert exc_info.value.status_code == 400
        assert exc_info.value.detail["code"] == SandboxErrorCodes.INVALID_PARAMETER
