...
def test_expire_pauses_running_sandbox():
    """TTL expiry should pause a running container but keep the sidecar alive."""
    service = DockerSandboxService(config=_app_config())
    mock_container = MagicMock()
    mock_container.attrs = {"State": {"Running": True, "Paused": False}, "Config": {"Labels": {}}}
    ...
    ) as mock_remove, patch.object(service, "_cleanup_egress_sidecar") as mock_cleanup:
...
def test_expire_skips_already_paused_sandbox():
    """TTL expiry on an already-paused container should just clear tracking."""
    service = DockerSandboxService(config=_app_config())
    mock_container = MagicMock()
    mock_container.attrs = {"State": {"Running": False, "Paused": True}, "Config": {"Labels": {}}}
