...
class TestAgentSandboxProviderEgress:
    ...
    def test_execd_token_stored_in_secret_not_annotation(self, mock_k8s_client):
        """Execd token should be stored in K8s Secret, not CR annotation."""
        mock_api = mock_k8s_client.get_custom_objects_api()
        ...
        provider = AgentSandboxProvider(mock_k8s_client, enable_informer=False)
        expires_at = datetime(2025, 12, 31, 10, 0, 0, tzinfo=timezone.utc)
        provider.create_workload(
            sandbox_id="test-id",
            namespace="test-ns",
        ...
        # Token should NOT be in CR annotation
    ...
    def test_get_execd_token_reads_from_secret(self, mock_k8s_client):
        """get_execd_token should read from K8s Secret."""
        import base64
        mock_core_api = mock_k8s_client.get_core_v1_api()
        mock_secret = MagicMock()
        mock_secret.data = {"token": base64.b64encode(b"mytoken").decode()}
        mock_core_api.read_namespaced_secret.return_value = mock_secret

        provider = AgentSandboxProvider(mock_k8s_client, enable_informer=False)
        result = provider.get_execd_token("test-id", "test-ns")

        assert result == "mytoken"
        mock_core_api.read_namespaced_secret.assert_called_once_with(
            name="test-id-execd-token", namespace="test-ns"
        )

    def test_delete_workload_cleans_up_execd_token_secret(self, mock_k8s_client):
        """delete_workload should clean up execd token Secret."""
        mock_api = mock_k8s_client.get_custom_objects_api()
        mock_core_api = mock_k8s_client.get_core_v1_api()
        ...
        mock_core_api.delete_namespaced_secret.assert_any_call(
            name="test-id-egress-token", namespace="test-ns"
        )
