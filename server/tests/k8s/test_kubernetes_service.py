...
class TestWaitForSandboxReady:
    ...
    def test_wait_for_pending_then_running_succeeds(self, k8s_service, mock_workload):
        ...
        result = k8s_service._wait_for_sandbox_ready("test-sandbox-id", timeout_seconds=10, fast_poll_interval_seconds=0.1)
    ...
    def test_wait_timeout_raises_exception(self, k8s_service, mock_workload):
        ...
        with pytest.raises(HTTPException) as exc_info:
            k8s_service._wait_for_sandbox_ready("test-sandbox-id", timeout_seconds=1, fast_poll_interval_seconds=0.5)
