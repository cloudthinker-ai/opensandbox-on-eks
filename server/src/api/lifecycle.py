...
async def create_sandbox(
    ...
    return await loop.run_in_executor(None, sandbox_service.create_sandbox, request)
...
async def pause_sandbox(
    ...
    # Run in executor to avoid blocking the async event loop (keeps /health responsive)
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, sandbox_service.pause_sandbox, sandbox_id)
...
async def resume_sandbox(
    ...
    # Run in executor to avoid blocking the async event loop (keeps /health responsive)
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, sandbox_service.resume_sandbox, sandbox_id)
    return Response(status_code=status.HTTP_202_ACCEPTED)
...
async def proxy_sandbox_endpoint_request(request: Request, sandbox_id: str, port: int, full_path: str):
    ...
    try:
        ...
        # Inject execd access token for authentication
        execd_token = sandbox_service.get_execd_token(sandbox_id)
        if execd_token:
            headers["X-EXECD-ACCESS-TOKEN"] = execd_token
