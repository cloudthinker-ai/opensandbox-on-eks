...
class NetworkRule(BaseModel):
    ...
    action: Literal["allow", "deny"] = Field(..., description="Whether to allow or deny matching targets.")
    target: str = Field(
        ...,
...
class NetworkPolicy(BaseModel):
    ...
    default_action: Optional[Literal["allow", "deny"]] = Field(
        default=None,
        alias="defaultAction",
        description="Default action when no egress rule matches. If omitted, sidecar defaults to deny.",
    )
...
class Sandbox(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(None, alias="expiresAt", description="Timestamp when sandbox will auto-pause. Null when sandbox is paused with no active TTL.")
    created_at: datetime = Field(..., alias="createdAt", description="Sandbox creation timestamp")
    ...
    def _always_include_expires_at(self, handler):
        """Ensure expiresAt is always present (as null when not set), even with exclude_none."""
        data = handler(self)
        # Determine the key name based on whether aliases are used
        key = "expiresAt" if "createdAt" in data else "expires_at"
        if key not in data:
            data[key] = None
        return data
