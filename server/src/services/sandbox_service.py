...
class SandboxService(ABC):
    ...
    def startup_stale_sandbox_sweep(self) -> None:
        """Run a one-time sweep for stale sandboxes after startup.

        The default implementation is a no-op.  Subclasses that manage
        long-lived workloads (e.g. Kubernetes) override this to detect
        and force-pause sandboxes whose operator-controlled images are
        out of date.
        """
    ...
    def get_execd_token(self, sandbox_id: str) -> Optional[str]:
        """
        Get the execd access token for a sandbox.

        Returns:
            Optional[str]: The token, or None if not available.
        """
        return None
