...
class DockerSandboxService(SandboxService):
    ...
    def _expire_sandbox(self, sandbox_id: str) -> None:
        """Timer callback to pause expired sandboxes instead of terminating them."""
        try:
        ...
        if state.get("Paused", False):
            # Already paused — just clear tracking
            logger.info("Sandbox %s already paused at TTL expiry; clearing timer.", sandbox_id)
            self._remove_expiration_tracking(sandbox_id)
            return
        ...
        try:
            if state.get("Running", False):
                container.pause()
                logger.info("Sandbox %s paused on TTL expiry.", sandbox_id)
            else:
                logger.warning(
                    "Sandbox %s is not running at TTL expiry (state=%s); skipping pause.",
                    sandbox_id,
                    state.get("Status", "unknown"),
                )
        except DockerException as exc:
            logger.warning("Failed to pause expired sandbox %s: %s", sandbox_id, exc)
    ...
    def _restore_existing_sandboxes(self) -> None:
        ...
        for container in containers:
            ...
            # Skip scheduling expiration for already-paused containers
            container_state = container.attrs.get("State", {})
            if container_state.get("Paused", False):
                logger.info(
                    "Sandbox %s is paused; skipping expiration scheduling.", sandbox_id
                )
                continue

            if expires_at <= now:
                logger.info("Sandbox %s already expired; pausing now.", sandbox_id)
    ...
    def _container_to_sandbox(self, container, sandbox_id: Optional[str] = None) -> Sandbox:
        ...
        expires_at = (
            None if paused
            else self._get_tracked_expiration(resolved_id, labels, created_at)
        )
    ...
    def pause_sandbox(self, sandbox_id: str) -> None:
        ...
        # Cancel any pending TTL timer so it doesn't fire while paused
        self._remove_expiration_tracking(sandbox_id)
