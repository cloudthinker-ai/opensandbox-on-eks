...
from src.services.constants import LABEL_PREFIX, SandboxErrorCodes
...
RESERVED_LABEL_PREFIXES = (LABEL_PREFIX,)
...
def ensure_metadata_labels(metadata: Optional[Dict[str, str]]) -> None:
    ...
    if not metadata:
        return
    for key in metadata:
        if any(key.startswith(prefix) for prefix in RESERVED_LABEL_PREFIXES):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "code": SandboxErrorCodes.INVALID_METADATA_LABEL,
                    "message": f"Metadata key '{key}' uses a reserved prefix.",
                },
            )
