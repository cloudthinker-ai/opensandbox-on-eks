...
class WorkloadProvider(ABC):
    ...
    def create_workload(
        ...
        Args:
            ...
            upstream_dns: Optional upstream DNS override for egress sidecar.
    ...
    def pause_workload(self, sandbox_id: str, namespace: str) -> None:
        """
        Pause a workload (scale to 0 replicas, preserve storage).
    ...
    def resume_workload(
        ...
        """
        Resume a paused workload (scale back to 1 replica).

        Args:
            sandbox_id: Unique sandbox identifier
            namespace: Kubernetes namespace
            network_policy: Optional current network policy to inject/update egress sidecar
            egress_image: Optional egress sidecar image for injection/update
    ...
    def get_execd_token(self, sandbox_id: str, namespace: str) -> Optional[str]:
        """Read the execd access token for a sandbox. Returns None by default."""
        return None

    def get_egress_token(self, sandbox_id: str, namespace: str) -> Optional[str]:
        """Read the egress auth token for a sandbox. Returns None by default."""
        return None

    @property
    def image_config_hash(self) -> Optional[str]:
        """Return the cached image config hash, or None if not supported."""
        return None
