...
class ServerConfig(BaseModel):
    ...
    proxy_auth_required: bool = Field(
        default=False,
        description="When true, proxy routes require API key authentication.",
    )
...
class KubernetesRuntimeConfig(BaseModel):
    ...
    storage_class: Optional[str] = Field(
        default=None,
        description="StorageClass for sandbox PVCs. If not set, uses cluster default.",
    )
    storage_size: str = Field(
        default="20Gi",
        description="Size of the sandbox persistent volume.",
    )
    snapshot_enabled: bool = Field(
        default=False,
        description="Enable CSI VolumeSnapshot support for cross-zone durability.",
    )
    snapshot_class: Optional[str] = Field(
        default=None,
        description="VolumeSnapshotClass name. Required when snapshot_enabled is true.",
    )
    snapshot_max_retention: int = Field(
        default=1,
        ge=1,
        description="Maximum number of VolumeSnapshots to retain per sandbox.",
    )
    archive_after_seconds: int = Field(
        default=86400,
        ge=0,
        description=(
            "Seconds after snapshot-ready before deleting the PVC (archive phase). "
            "Set to 0 to delete PVC immediately after snapshot completes. "
            "Only applies when snapshot_enabled is true."
        ),
    )
    filesystem_persistence: bool = Field(
        default=False,
        description=(
            "Enable full filesystem persistence via OverlayFS. "
            "When true, all writes to any path survive pause/resume. "
            "Requires CAP_SYS_ADMIN on the sandbox container."
        ),
    )
    sandbox_user: Optional[str] = Field(
        default="user",
        description=(
            "Non-root username to drop privileges to after OverlayFS setup. "
            "bootstrap.sh switches to this user before running the entrypoint. "
            "Set to None to stay as root (not recommended)."
        ),
    )
...
class EgressConfig(BaseModel):
    ...
    network_access_config_file: Optional[str] = Field(
        default=None,
        description="Path to YAML file with scoped network access policies.",
    )
    upstream_dns: Optional[str] = Field(
        default=None,
        description="Override upstream DNS for egress sidecar (e.g. '172.20.0.10:53').",
    )
...
class RuntimeConfig(BaseModel):
    ...
    eager_image_update: bool = Field(
        default=True,
        description=(
            "When true, force-pause running sandboxes with stale operator-controlled "
            "images on server startup so they pick up new images on next resume."
        ),
    )
    startup_sweep_timeout: int = Field(
        default=5,
        ge=0,
        le=300,
        description=(
            "Maximum seconds to wait for old pods to drain before running "
            "the stale-sandbox sweep. The sweep runs immediately once this "
            "pod is the only Ready server pod, or after this timeout."
        ),
    )
