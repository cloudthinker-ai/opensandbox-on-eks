...
func InitFlags() {
	...
	if v := os.Getenv(accessTokenEnv); v != "" && ServerAccessToken == "" {
		ServerAccessToken = v
	}

	// Read token from file (preferred over env var for security)
	if ServerAccessToken == "" {
		if tokenFile := os.Getenv(accessTokenFileEnv); tokenFile != "" {
			data, err := os.ReadFile(tokenFile)
			if err != nil {
				stdlog.Panicf("failed to read access token file %s: %v", tokenFile, err)
			}
			ServerAccessToken = strings.TrimSpace(string(data))
			os.Remove(tokenFile) // delete so token only lives in memory
		}
	}

	// Clear token env vars so they don't leak via /proc/self/environ
	os.Unsetenv(accessTokenEnv)
	os.Unsetenv(accessTokenFileEnv)
