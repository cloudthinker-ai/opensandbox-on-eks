...
// DNS rebinding guard: resolved IPs that match deny rules are filtered out before
// being added to the dynamic allow set, preventing attackers from smuggling private
// IPs into the allow set via controlled DNS responses.
func (p *Proxy) maybeNotifyResolved(domain string, resp *dns.Msg) {
	...
	// Bypass rebinding guard when the user explicitly allowed this domain.
	// Their allow rule signals intent to reach whatever IPs it resolves to,
	// even private ones that would normally be blocked by deny CIDRs.
	explicitlyAllowed := currentPolicy != nil && currentPolicy.IsExplicitlyAllowedDomain(domain)

	// Filter out IPs that match deny rules (DNS rebinding guard)
	filtered := ips[:0]
	for _, rip := range ips {
		if !explicitlyAllowed && currentPolicy != nil && currentPolicy.IsDeniedIP(rip.Addr) {
			log.Warnf("[dns] rebinding guard: dropping resolved IP %s for domain %s (matches deny rule)", rip.Addr, domain)
			continue
		}
		filtered = append(filtered, rip)
	}
	if len(filtered) == 0 {
		return
	}
	p.onResolved(domain, filtered)
}
...
func discoverUpstream() (string, error) {
	if override := strings.TrimSpace(os.Getenv(constants.EnvEgressUpstream)); override != "" {
		host, _, err := net.SplitHostPort(override)
		if err != nil {
			return "", fmt.Errorf("invalid %s=%q: %w", constants.EnvEgressUpstream, override, err)
		}
		if net.ParseIP(host) == nil {
			return "", fmt.Errorf("invalid %s=%q: host %q is not a valid IP", constants.EnvEgressUpstream, override, host)
		}
		log.Infof("[dns] upstream override via %s: %s", constants.EnvEgressUpstream, override)
		return override, nil
	}
