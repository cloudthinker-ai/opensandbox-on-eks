...
func TestUpstreamOverride_EnvVar(t *testing.T) {
	t.Setenv("OPENSANDBOX_EGRESS_UPSTREAM", "1.2.3.4:53")
	proxy, err := New(nil, "127.0.0.1:15353")
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	if got := proxy.UpstreamHost(); got != "1.2.3.4" {
		t.Fatalf("UpstreamHost() = %q, want %q", got, "1.2.3.4")
	}
}

func TestUpstreamOverride_InvalidEnv(t *testing.T) {
	t.Setenv("OPENSANDBOX_EGRESS_UPSTREAM", "1.2.3.4")
	_, err := New(nil, "127.0.0.1:15353")
	if err == nil {
		t.Fatal("expected error for env var without port, got nil")
	}
}
...
func TestMaybeNotifyResolved_FiltersRebind(t *testing.T) {
	// Policy denies 10.0.0.0/8; resolved IPs in that range should be filtered out
	pol, _ := policy.ParsePolicy(`{"defaultAction":"allow","egress":[{"action":"deny","target":"10.0.0.0/8"}]}`)
	proxy, err := New(pol, "")
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	ch := make(chan []nftables.ResolvedIP, 1)
	proxy.SetOnResolved(func(_ string, ips []nftables.ResolvedIP) {
		ch <- ips
	})

	msg := new(dns.Msg)
	msg.Answer = []dns.RR{
		&dns.A{Hdr: dns.RR_Header{Name: "evil.com.", Ttl: 60}, A: net.ParseIP("10.0.0.1")},
		&dns.A{Hdr: dns.RR_Header{Name: "evil.com.", Ttl: 60}, A: net.ParseIP("1.2.3.4")},
	}
	proxy.maybeNotifyResolved("evil.com.", msg)

	select {
	case got := <-ch:
		if len(got) != 1 {
			t.Fatalf("expected 1 IP after filtering, got %d: %v", len(got), got)
		}
		if got[0].Addr.String() != "1.2.3.4" {
			t.Fatalf("expected 1.2.3.4, got %s", got[0].Addr)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("callback was not invoked")
	}
...
func TestMaybeNotifyResolved_AllFilteredNoCallback(t *testing.T) {
	// When all resolved IPs match deny rules, callback should not be invoked
	pol, _ := policy.ParsePolicy(`{"defaultAction":"allow","egress":[{"action":"deny","target":"10.0.0.0/8"}]}`)
	proxy, err := New(pol, "")
	if err != nil {
		t.Fatalf("New: %v", err)
	...
	if called {
		t.Fatal("callback should not be invoked when all IPs are filtered")
	}
}

func TestMaybeNotifyResolved_BypassesRebindForExplicitAllow(t *testing.T) {
	// User explicitly allows rds.amazonaws.com; it resolves to 10.2.58.106 which
	// matches deny CIDR 10.0.0.0/8. The rebinding guard should be bypassed.
	pol, _ := policy.ParsePolicy(`{
		"defaultAction":"allow",
		"egress":[
	...
	msg := new(dns.Msg)
	msg.Answer = []dns.RR{
		&dns.A{Hdr: dns.RR_Header{Name: "rds.amazonaws.com.", Ttl: 60}, A: net.ParseIP("10.2.58.106")},
	}
	proxy.maybeNotifyResolved("rds.amazonaws.com.", msg)

	select {
	case got := <-ch:
		if len(got) != 1 || got[0].Addr.String() != "10.2.58.106" {
			t.Fatalf("expected [10.2.58.106], got %v", got)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("callback was not invoked; rebinding guard incorrectly blocked explicit allow")
	}
}

func TestMaybeNotifyResolved_RebindStillActiveForDefaultAllow(t *testing.T) {
	// other.com is allowed only via defaultAction:"allow" (no explicit rule).
	// Resolving to 10.0.0.1 should still be blocked by rebinding guard.
	pol, _ := policy.ParsePolicy(`{
		"defaultAction":"allow",
		"egress":[
			{"action":"allow","target":"rds.amazonaws.com"},
	...
	msg := new(dns.Msg)
	msg.Answer = []dns.RR{
		&dns.A{Hdr: dns.RR_Header{Name: "other.com.", Ttl: 60}, A: net.ParseIP("10.0.0.1")},
	}
	proxy.maybeNotifyResolved("other.com.", msg)

	if called {
		t.Fatal("callback should not be invoked; rebinding guard should block default-allow domain resolving to denied IP")
	}
}

func TestMaybeNotifyResolved_WildcardExplicitAllow(t *testing.T) {
	// Wildcard allow rule *.rds.amazonaws.com should bypass rebinding guard
	// for us-east-1.rds.amazonaws.com resolving to 10.2.58.106.
	pol, _ := policy.ParsePolicy(`{
		"defaultAction":"allow",
		"egress":[
			{"action":"allow","target":"*.rds.amazonaws.com"},
	...
	msg := new(dns.Msg)
	msg.Answer = []dns.RR{
		&dns.A{Hdr: dns.RR_Header{Name: "us-east-1.rds.amazonaws.com.", Ttl: 60}, A: net.ParseIP("10.2.58.106")},
	}
	proxy.maybeNotifyResolved("us-east-1.rds.amazonaws.com.", msg)
	...
	case <-time.After(2 * time.Second):
		t.Fatal("callback was not invoked; rebinding guard incorrectly blocked wildcard explicit allow")
	}
}
