...
func TestApplyStatic_BuildsRuleset_DefaultDeny(t *testing.T) {
	...
	expectContains(t, rendered, "add chain inet opensandbox egress { type filter hook output priority -150; policy drop; }")
	...
	expectContains(t, rendered, "add rule inet opensandbox egress meta nfproto ipv6 drop")
...
func TestApplyStatic_AllowBeforeDenyRuleOrder(t *testing.T) {
	var rendered string
	m := NewManagerWithRunner(func(_ context.Context, script string) ([]byte, error) {
		rendered = script
		return nil, nil
	...
	p, _ := policy.ParsePolicy(`{
		"defaultAction":"allow",
		"egress":[
			{"action":"allow","target":"192.168.65.254"},
	...
	// Verify allow rules come before deny rules (first match wins in nftables)
	allowIdx := strings.Index(rendered, "ip daddr @allow_v4 accept")
	denyIdx := strings.Index(rendered, "ip daddr @deny_v4 drop")
	dynAllowIdx := strings.Index(rendered, "ip daddr @dyn_allow_v4 accept")

	if allowIdx < 0 || denyIdx < 0 || dynAllowIdx < 0 {
		t.Fatalf("missing expected rules in rendered script:\n%s", rendered)
	}
	if allowIdx > dynAllowIdx {
		t.Fatalf("allow_v4 should come before dyn_allow_v4 (allow=%d, dyn=%d)", allowIdx, dynAllowIdx)
	}
	if dynAllowIdx > denyIdx {
		t.Fatalf("dyn_allow_v4 should come before deny_v4 (dyn=%d, deny=%d)", dynAllowIdx, denyIdx)
	}
}
