...
func buildRuleset(p *policy.NetworkPolicy, opts Options) string {
	...
	fmt.Fprintf(&b, "add chain inet %s %s { type filter hook output priority -150; policy %s; }\n", tableName, chainName, chainPolicy)
	...
	// Drop all IPv6 egress (loopback already accepted above).
	// This replaces the previous approach of disabling IPv6 via kernel sysctls,
	// which is not allowed by EKS (unsafe sysctls).
	...
	// Allow rules first: static allow → dynamic allow (DNS-learned) → deny.
	// nftables evaluates rules in order (first match wins), and the DNS rebinding
	// guard (A3) prevents denied IPs from entering dyn_allow, so this is safe.
