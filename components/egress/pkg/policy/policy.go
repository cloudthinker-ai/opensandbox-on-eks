...
// IsExplicitlyAllowedDomain returns true if the domain matches an explicit
// allow rule in the egress list. Domains allowed only via DefaultAction
// return false. This lets callers bypass the DNS rebinding guard for
// user-intentional private-IP destinations while keeping it active for
// domains that are merely not-denied.
func (p *NetworkPolicy) IsExplicitlyAllowedDomain(domain string) bool {
	if p == nil {
		return false
	}
	domain = strings.ToLower(strings.TrimSuffix(domain, "."))
...
// IPs already covered by an existing allow CIDR rule are skipped to avoid nftables "conflicting intervals".
...
// IsDeniedIP returns true if addr matches any deny IP or CIDR rule.
func (p *NetworkPolicy) IsDeniedIP(addr netip.Addr) bool {
	if p == nil {
		return false
	}
	for _, r := range p.Egress {
...
// IsAllowedByCIDR returns true if addr falls within any allow CIDR rule.
func (p *NetworkPolicy) IsAllowedByCIDR(addr netip.Addr) bool {
	if p == nil {
		return false
	}
	for _, r := range p.Egress {
		if r.Action == ActionAllow && r.targetKind == targetCIDR && r.prefix.Contains(addr) {
			return true
		}
	}
	return false
