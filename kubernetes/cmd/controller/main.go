...
func main() {
	...
	flag.StringVar(&serverBaseURL, "server-base-url", "", "Base URL of the OpenSandbox server for pause API calls")
	var serverAPIKey string
	flag.StringVar(&serverAPIKey, "server-api-key", "", "API key for authenticating with the OpenSandbox server")
	...
	batchReconciler := &controller.BatchSandboxReconciler{
	...
	if serverBaseURL != "" {
		batchReconciler.SetHTTPClient(&http.Client{Timeout: 10 * time.Second})
	}
