...
var _ = Describe("BatchSandbox Controller", func() {
	...
	Context("When create new batch sandbox, create pod base on pod template", func() {
		...
		It("should pause batch sandbox on TTL expiry instead of deleting it", func() {
			...
			Eventually(
				func(g Gomega) {
					bs := &sandboxv1alpha1.BatchSandbox{}
					g.Expect(k8sClient.Get(ctx, typeNamespacedName, bs)).To(Succeed())
					// CR should still exist with paused annotation
					g.Expect(bs.Annotations[AnnotationSandboxPaused]).To(Equal("true"))
					// Replicas should be scaled to 0
					g.Expect(*bs.Spec.Replicas).To(Equal(int32(0)))
					// ExpireTime should be cleared
					g.Expect(bs.Spec.ExpireTime).To(BeNil())
					// Pods should be scaled down to 0
					allPods := &corev1.PodList{}
					g.Expect(k8sClient.List(ctx, allPods, &client.ListOptions{Namespace: bs.Namespace})).Should(Succeed())
					pods := []*corev1.Pod{}
					for i := range allPods.Items {
