...
type BatchSandboxReconciler struct {
	...
	ServerBaseURL string       // OpenSandbox server URL for pause API
	ServerAPIKey  string       // API key for authenticating with server
	httpClient    *http.Client // Reused HTTP client for pause API calls
}
	...
	if expireAt := batchSbx.Spec.ExpireTime; expireAt != nil {
		...
		if expireAt.Time.Before(now) {
			if batchSbx.DeletionTimestamp == nil {
				// Pool-based sandboxes: keep existing delete behavior (pause not supported)
				if batchSbx.Spec.PoolRef != "" {
					log.Info("batch sandbox expired, delete (pool-based)", "expireAt", expireAt)
				...
				} else if batchSbx.Annotations[AnnotationSandboxPaused] == "true" {
					// Already paused: just clear expireTime to stop re-triggering
					log.Info("batch sandbox expired but already paused, clearing expireTime", "expireAt", expireAt)
					...
					// Running sandbox: pause instead of delete
					log.Info("batch sandbox expired, pausing", "expireAt", expireAt)
					if r.ServerBaseURL != "" {
						if err := r.callPauseAPI(ctx, batchSbx.Name); err != nil {
							log.Error(err, "pause API call failed, falling back to direct patch")
						} else {
							// API handled pause + snapshot; clear expireTime to stop re-triggering
							patchData, _ := json.Marshal(map[string]any{
								"spec": map[string]any{
									"expireTime": nil,
								},
					...
					// Fallback: direct CRD patch (no snapshot)
					patchData, _ := json.Marshal(map[string]any{
						"metadata": map[string]any{
							"annotations": map[string]string{
								AnnotationSandboxPaused: "true",
							},
						},
						"spec": map[string]any{
							"replicas":   0,
...
func (r *BatchSandboxReconciler) scaleBatchSandbox(ctx context.Context, batchSandbox *sandboxv1alpha1.BatchSandbox, podTemplateSpec *corev1.PodTemplateSpec, pods []*corev1.Pod) error {
	...
	// Observe delete expectations for pods that are already gone
	controllerKey := controllerutils.GetControllerKey(batchSandbox)
	if pendingExpectations := BatchSandboxScaleExpectations.GetExpectations(controllerKey); pendingExpectations != nil {
		if pendingDeletes, ok := pendingExpectations[expectations.Delete]; ok {
			for _, podName := range pendingDeletes.List() {
				if !activePodNames[podName] {
					BatchSandboxScaleExpectations.ObserveScale(controllerKey, expectations.Delete, podName)
				}
			}
		}
	}
	...
	// Find pods to delete for scale-down (index >= desired replicas)
	for idx, pod := range indexedPodMap {
	...
	if len(needDeletePods) > 0 {
		log.Info("try to delete Pods for scale-down", "count", len(needDeletePods))
	}
	for _, pod := range needDeletePods {
		...
		if err := r.Delete(ctx, pod); err != nil {
			...
			if !errors.IsNotFound(err) {
				r.Recorder.Eventf(batchSandbox, corev1.EventTypeWarning, "FailedDelete", "failed to delete pod: %v", err)
				return err
			}
		} else {
			r.Recorder.Eventf(batchSandbox, corev1.EventTypeNormal, "SuccessfulDelete", "deleted pod %s for scale-down", pod.Name)
		}
	}
	return nil
}
...
// SetHTTPClient sets the HTTP client used for pause API calls.
// Must be called before SetupWithManager to avoid data races.
func (r *BatchSandboxReconciler) SetHTTPClient(c *http.Client) {
	r.httpClient = c
}

func (r *BatchSandboxReconciler) callPauseAPI(ctx context.Context, sandboxID string) error {
	url := fmt.Sprintf("%s/sandboxes/%s/pause", r.ServerBaseURL, sandboxID)
	req, err := http.NewRequestWithContext(ctx, "POST", url, nil)
	if err != nil {
		return err
	}
	if r.ServerAPIKey != "" {
		req.Header.Set("OPEN-SANDBOX-API-KEY", r.ServerAPIKey)
	}
	...
	if resp.StatusCode != http.StatusAccepted {
		...
		return fmt.Errorf("pause API returned status %d: %s", resp.StatusCode, string(body))
	}
	return nil
}
